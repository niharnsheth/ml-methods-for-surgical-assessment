...
for file in csv_list_1:
    ...
    # Store separate dataframes or arrays based on sensor id
    ...
    zero_value_numpy = 2 - list_length
    list_zero_numpy = 0

    # Calculate permutations:
    # poss_permutations = factorial_pos_no(total_no_sensors) / factorial_pos_no(total_no_sensors - list_length)
    if zero_value_numpy == 1:
       list_zero_numpy = np.zeros((longest_data_sequence_length, features_per_sensor))
    ...
    if zero_value_numpy == 1:
        final_numpy = np.hstack((final_numpy, list_pos_numpy[0]))
        final_numpy = np.hstack((final_numpy, list_ori_numpy[0]))
        final_numpy = np.hstack((final_numpy, list_zero_numpy))
    elif zero_value_numpy == 0:
        final_numpy = np.hstack((final_numpy, list_pos_numpy[0]))
        final_numpy = np.hstack((final_numpy, list_ori_numpy[0]))
        final_numpy = np.hstack((final_numpy, list_pos_numpy[1]))
        final_numpy = np.hstack((final_numpy, list_ori_numpy[1]))
