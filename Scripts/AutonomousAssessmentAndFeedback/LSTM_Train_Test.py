...
no_of_actions = 2
csv_list = [f for f in os.listdir(source_path + input_folder + surgery_name_list[surgery_selected])
...
n_classes = 5 #number of outputs for classification
epochs = 10
learning_rate = 0.001

n_units = 120 # number of lstm cells
...
def create_motion_windows(no_windows, df_to_change):
    ...
    for step in steps:#range(len(df_to_change) - no_windows + 1):
        ...
        a = df_to_change.iloc[step:step+no_windows, :-5].reset_index(drop=True).to_numpy()
