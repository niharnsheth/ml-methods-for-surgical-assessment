...
# vel_ang = (0.2,-0.2)
#
# #df = pd.read_csv(source_path + data_folder + csv_list[0], index_col=0)
# df = pd.read_csv(source_path + data_folder + train_list[0], index_col=0)
...
def create_motion_windows(no_windows, df_to_change):
    ...
    for i in steps:#range(len(df_to_change) - no_windows + 1):
        ...
        a = df.iloc[i:i+no_windows, :-3].reset_index(drop=True).to_numpy()
        # a.reset_index(drop=True)
        b = df.iloc[i+no_windows, 13:].reset_index(drop=True).to_numpy()
...
#         shuffle_list = list(zip(feature_list, label_list))
#         random.shuffle(shuffle_list)
#         feature_list, label_list = zip(*shuffle_list)
